...
def controlled_round(group, value_col):
    values = group[value_col]

    floored = np.floor(values)
    remainder = values - floored

    target = round(values.sum())
    deficit = int(target - floored.sum())

    result = floored.copy()

    if deficit > 0:
        top_idx = remainder.nlargest(deficit).index
...
bkr_du_df = pd.read_csv(os.path.join(working_folder, bkr_du_file))
...
juris_in_bkr_du = bkr_du_df['Jurisdiction'].unique()
bkr_total_parcels_df = hhs_by_parcel_df.loc[hhs_by_parcel_df['Jurisdiction'].isin(juris_in_bkr_du)]
bkr_parcels_provided = bkr_du_df.shape[0]
if bkr_total_parcels_df.shape[0] != bkr_parcels_provided:
    print('BKR forecast does not cover all parcels. Please cehck the missing parcel files for further investigation.')
    bkr_missing_parcels_df = bkr_total_parcels_df.loc[~bkr_total_parcels_df['PSRC_ID'].isin(bkr_du_df['PSRC_ID'])]
    bkr_missing_parcels_df.to_csv(os.path.join(working_folder, 'bkr_missing_parcels.csv'), index = False)
    print(f'{bkr_missing_parcels_df.shape[0]} parcels are missing in {bkr_du_file}.')

bkr_du_df['sfhhs'] = bkr_du_df['SFUnits'] * sf_occupancy_rate 
bkr_du_df['mfhhs'] = bkr_du_df['MFUnits'] * mf_occupancy_rate
bkr_du_df['sfpersons'] = bkr_du_df['sfhhs'] * avg_persons_per_sfhh
bkr_du_df['mfpersons'] = bkr_du_df['mfhhs'] * avg_persons_per_mfhh
bkr_du_df['cobflag'] = 'bkr'

adjusted_hhs_by_parcel_df = adjusted_hhs_by_parcel_df.merge(bkr_du_df[['PSRC_ID', 'cobflag', 'sfhhs', 'mfhhs', 'sfpersons', 'mfpersons']], on = 'PSRC_ID', how = 'left')
# reset hhs and persons in all COB parcels to zero. Only use local forecast.
adjusted_hhs_by_parcel_df.loc[adjusted_hhs_by_parcel_df['Jurisdiction'].isin(juris_in_bkr_du), ['adj_hhs_by_parcel', 'adj_persons_by_parcel']] = 0
...
adjusted_hhs_by_parcel_df.loc[adjusted_hhs_by_parcel_df['cobflag'] == 'bkr', 'adj_hhs_by_parcel'] = adjusted_hhs_by_parcel_df['sfhhs'] + adjusted_hhs_by_parcel_df['mfhhs']
adjusted_hhs_by_parcel_df.loc[adjusted_hhs_by_parcel_df['cobflag'] == 'bkr', 'adj_persons_by_parcel'] = adjusted_hhs_by_parcel_df['sfpersons'] + adjusted_hhs_by_parcel_df['mfpersons']
...
for row in special_hhs_by_TAZ.itertuples():
    ...
    if mf_parcels_count > row.adj_hhs_by_parcel:
        ...
        adjusted_hhs_by_parcel_df.loc[adjusted_hhs_by_parcel_df['PSRC_ID'].isin(selected_ids), 'adj_hhs_by_parcel'] += 1
    else:
        increase = math.floor(row.adj_hhs_by_parcel / mf_parcels_count)
        adjusted_hhs_by_parcel_df.loc[mf_parcels_flag, 'adj_hhs_by_parcel'] += increase
...
adjusted_hhs_by_parcel_df['adj_hhs_by_parcel'] = (
    adjusted_hhs_by_parcel_df
    .groupby('BKRCastTAZ', group_keys=False)
    .apply(lambda g: controlled_round(g, 'adj_hhs_by_parcel'))
)
...
bkr_parcels_du_df = bkr_total_parcels_df[['PSRC_ID']].merge(bkr_du_df[['PSRC_ID', 'SFUnits', 'MFUnits']], on = 'PSRC_ID', how = 'left').fillna(0)
