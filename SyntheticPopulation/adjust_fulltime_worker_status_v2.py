...
### input configuration
working_folder = r"I:\Modeling and Analysis Group\01_BKRCast\BKRPopSim\PopulationSim_BaseData\2044_long_term_plan\20%WFH"
original_h5_file_name = '2044_long_term_hh_and_persons.h5'
...
adjustment_factor_name = r"TAZ_subarea_worker_adjustment_20%.csv"

### output configuration
updated_h5_file_name = '2044_long_term_hh_and_persons_forWFH_20%.h5'
...
for taz in adjustment_factor_df.itertuples():
    ...
    persons_in_taz = updated_person_df.loc[(updated_person_df['hhtaz'] == taz.BKRCastTAZ)]
    ...
    if fulltime_workers_in_taz.shape[0] > 0:
        ...
        persons_in_selected_mask = updated_person_df['pid'].isin(selected['pid'])
        updated_person_df.loc[persons_in_selected_mask, 'pwtyp'] = 0

        # for pptyp == [1,2] in selected, set pptyp to 4..
        updated_person_df.loc[persons_in_selected_mask & (updated_person_df['pptyp'].isin([1, 2])), 'pptyp'] = 0
